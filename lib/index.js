...
function fetch(addresses, userId, sourceId, callback) {
  api.getAddressesTransactions(addresses, {limit: config.maxReturnTx}, function (err, resp) {
    ...
    log.trace({ transactionCount: resp.length }, 'Received transactions from Chain');

    var transactions = resp.map(function (chainTx) {
      return transformTransaction(chainTx, userId, sourceId);
    });
...
Chain.prototype.fetchTransactionsFromAddresses = function (addresses, callback) {
  log.debug({addressesCount: addresses.length, sourceId: this.sourceId, userId: this.userId},
  ...
  if (addresses.length <= config.addressChunkSize) {
    ...
    log.debug({count: Math.ceil(addresses.length / config.addressChunkSize)},
