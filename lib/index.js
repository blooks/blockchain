...
function transformTransaction(chainTx, userId, sourceId) {
  ...
  var inputs = chainTx.inputs.map(function (input) {
    if (input.addresses) {//ignore op returns
      return {
        amount: input.value,
  ...
  var outputs = chainTx.outputs.map(function (output) {
    if (output.addresses) {//Ignore op returns
      return {
        amount: output.value,
...
var Chain = function (userId, sourceId) {
  ...
  if (!process.env.CHAIN_API_KEY) throw new Error('You must specify a CHAIN_API_KEY environment variable to run examples');
  if (!process.env.CHAIN_API_SECRET) throw new Error('You must specify a CHAIN_API_SECRET environment variable to run examples');
  ...
  if (addresses.length <= self.config.addressChunkSize) {
    ...
    var childJobs = _.range(0, addresses.length, self.config.addressChunkSize).map(function (start) {
      ...
      return function (callback) {
        fetch(addresses.slice(start, end), self.userId, self.sourceId, callback);
      }.bind(self);
    }.bind(self));
