...
var log = require('@blooks/log').child({ component: 'Blockchain Data Lib' })
...
function transformTransaction (insightTx, userId, sourceId) {
  ...
  log.trace(coynoTx, 'Computed coyno style transaction')
  return coynoTx
}
...
BlockChainClient.prototype.fetchTransactionsFromAddresses = function (addresses, callback) {
  ...
  if (addresses.length <= self.config.addressChunkSize) {
    log.trace('Fetching addresses:', addresses)
    return self.fetch(addresses, self.userId, self.sourceId, callback)
  }
